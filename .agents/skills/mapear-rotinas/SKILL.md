...
description: >
  ...
  skills concretas e cria as aprovadas em `.agents/skills/`. Use quando o usuário pedir
...
Ler `templates/skills/catalogo.md` pra ver se alguma das tarefas mencionadas já é coberta por uma skill nativa do Codex ou validada pelo MazyOS. Se sim, sugerir a skill existente em vez de criar uma nova:
...
1. Criar pasta `.agents/skills/<nome>/`
...
```
Criei [N] skills:
✓ $<nome1> — em .agents/skills/<nome1>/SKILL.md
✓ $<nome2> — em .agents/skills/<nome2>/SKILL.md
...
